...
const NavBar: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      {/* Mobile Menu Overlay */}
      {isOpen && (
        <div className={`absolute top-full left-0 w-full md:hidden flex flex-col p-6 space-y-6 z-50 border-t rounded-b-2xl shadow-2xl transition-colors
          ${scrolled ? "bg-white text-black border-gray-100" : "bg-black/95 backdrop-blur-lg text-white border-white/10"}`}>
          {/* Mobile Links */}
          ...
          <div className="flex flex-col space-y-3 pt-4 border-t border-white/10">
            <a href="tel:+1234568910" className={`flex justify-center items-center space-x-2 px-5 py-3 rounded-full border-2 text-sm font-semibold transition-all
              ${scrolled ? "border-black/20" : "border-white/50"}`}>
              <Phone size={16} />
              <span>+123 456 8910</span>
            </a>
            <button className={`w-full px-5 py-3 rounded-full border-2 font-bold text-sm
